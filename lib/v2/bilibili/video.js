...
module.exports = async (ctx) => {
    ...
    const params = utils.addVerifyInfo(
        `mid=${uid}&ps=30&tid=0&pn=1&keyword=${keyword}&order=pubdate&platform=web&web_location=1550101&order_avoided=true&dm_img_list=[]&dm_img_str=${Buffer.from('no webgl').toString('base64').slice(0, -2)}&dm_cover_img_str=${Buffer.from(
            'no webgl'
        )
            .toString('base64')
            .slice(0, -2)}`,
